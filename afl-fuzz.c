...
void HWMmutate(u8** buf, u32* buf_len, s32 n_mutants){
  // FILE *fp;
  char tail[20];
  struct timeval time;
  gettimeofday(&time, NULL);
  sprintf(tail,"%d%ld",getpid(),time.tv_usec);
  char path1[128],path2[128],cmd[258];
  strcpy(path1,exPath);
  strcat(path1,"/Temp/tempA");
  strcpy(path2,exPath);
  strcat(path2,"/Temp/tempB");
  strcpy(cmd,exPath);
  strcat(cmd,"/aigmutate ");
  strcat(path1,tail);
  strcat(path1,".aig");
  strcat(path2,tail);
  strcat(path2,".aig");
  ...
  s32 fd;
  fd = open(path1,O_WRONLY | O_CREAT | O_EXCL,0600);
  s32 _res=write(fd,*buf,*buf_len);
  if(_res != *buf_len){
    ACTF("WriteFail:\n");
  }
  close(fd);
  ...
  *buf=mmap(0, *buf_len, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  if(*buf==MAP_FAILED){
    close(fd);
    return;
  }
...
#define SP20    SP10 SP10
  ...
  sprintf(tmp, "%s/%s, %s/%s",
          ...
          DI(stage_finds[STAGE_AIGFUZZ]), DI(stage_cycles[STAGE_AIGFUZZ]));
...
aigfuzz_mutation_stage:

    stage_name = "aigfuzz_mutation_stage";
    stage_short = "ams";
    goto retry_aigfuzz_mutation_stage;

  retry_aigfuzz_mutation_stage:
    stage_max = 8; // Dominik: putting this constant here     
    orig_hit_cnt =  queued_paths + unique_crashes;
    u8 *buf = in_buf;
    u32 buf_len = len;
    for (stage_cur = 0; stage_cur < stage_max; stage_cur++){            
      //  char temp[20];
      //  sprintf(temp,"%d",buf_len);
      //  printToFile(temp);
      //  printToFile("before\n");
        
       HWMmutate(&buf, &buf_len, n_mutants);
      //   sprintf(temp,"%d",buf_len);
      //   printToFile(temp);
      // printToFile("after\n");
       if (buf) { 
          if(buf_len>0){
            if (common_fuzz_stuff(argv, buf, buf_len)) {                
                //free(buf);
                munmap(buf,buf_len);
                goto abandon_entry;
            }
    ...
    stage_finds[STAGE_AIGFUZZ]  += new_hit_cnt - orig_hit_cnt;
    stage_cycles[STAGE_AIGFUZZ] += stage_max;
  /****************
   * RANDOM HAVOC *
   ****************/
    ret_val = 0;
...
int main(int argc, char** argv) {
  ...
  strncpy(exPath,argv[0],strlen(argv[0])-9);
  char** use_argv;
